...
node_colors_default = 'gray'
colors_nodes = [node_colors.get(G.nodes[node].get('placetype', node_colors_default)) for node in G.nodes]

# Prepare a color map for edges based on placetype
edge_colors = {'Bar': 'blue', 'Church': 'green'}
edge_colors_default = 'gray'
colors_edges = [edge_colors.get(G.edges[edge[0], edge[1], 0].get('placetype', edge_colors_default)) for edge in G.edges]

# Draw the graph with color-coded nodes and edges
pos = nx.spring_layout(G)  # You can use different layouts as per your preference
nx.draw(G, pos, with_labels=True, node_color=colors_nodes, font_size=8, font_color='black', font_weight='bold', node_size=3000, edge_color=colors_edges)
